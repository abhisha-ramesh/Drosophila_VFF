...
global delta
delta = 0.06            #growth rate for cells
#*******************************************************************************************************#
#INITIAL CONDITIONS
class Draw(SteppableBasePy):
    ...
    apical, core and basal compartments. Cells are surrounded by vitelline membrane. 
    Volume constraint for each of these compartments are also defined here.
    ...
    def start(self):
        ...
        nLayers = int(self.Radius/cd)           #layers
        n = 47                                  #number of cells
        dAng = pi/n                             #angular separation between cells
        rMax = int(cd/2)+0.5                    #maximum radius for the first layer
        
        #Apical, Basal, Core and Vitelline Membrane cell types are created in sequential layers 
        for Layer in range(nLayers-5):
            rMin = rMax
            rMax = int(sqrt(rMin*rMin + 5*tV/dAng))
        for Layer in range(nLayers-4,nLayers+2):
            cellList=[]
            if (Layer==nLayers-4):  
                type=self.BASAL
                rMin=rMax
            ...
            if (Layer==nLayers-3):  
                ...
                type=self.CORE
                rMin=rMax
                rMax=int(sqrt(rMin*rMin+tV*4.8/dAng))
            if (Layer==nLayers-1):  
                type=self.APICAL
                rMin=rMax
            ...
            for i in range(n):
                if(Layer==nLayers-4):
                    ...
                    elif(i%2 == 0):                               #and New Basal, New Basal Prime have same physical properties; 
            ...
            for cell in self.cellListByType(self.BASAL): #BASAL
                ...
                if (self.mid-9<= cell.id <=self.mid+9):
                    cell.lambdaVolume=self.LamV_bas/4.0
            ...
            for cell in self.cellListByType(self.APICAL): #APICAL
                ...
                if ((3*self.tot)+self.mid-9<= cell.id <=(3*self.tot)+self.mid+9):
                ...
                elif (cell.id == (3*self.tot)+1) or (cell.id == 4*self.tot):
                    cell.lambdaVolume=self.LamV*1000
                elif (cell.id == (3*self.tot)+2) or (cell.id == (4*self.tot)-1):
                    cell.lambdaVolume=self.LamV
                else:
                    cell.lambdaVolume=self.LamV*50
                ap_list.append(cell.id)
            
            for cell in self.cellListByType(self.VM): #VITELLINE MEMBRANE
                cell.targetVolume=cell.volume + 0.5
                cell.lambdaVolume = 1000
            
        #Group each apical, core and basal domains into corresponding cell/cluster
        for cell in self.cellListByType(self.BASAL): #BASAL
            new_bas_cell = self.fetch_cell_by_id(cell.id+n)
            core_cell = self.fetch_cell_by_id(cell.id+(2*n))
            ap_cell = self.fetch_cell_by_id(cell.id+(3*n))
            self.reassign_cluster_id(core_cell, cell.clusterId)
            self.reassign_cluster_id(ap_cell, core_cell.clusterId)
            self.reassign_cluster_id(new_bas_cell, core_cell.clusterId)
        ...
        if (mcs <= 26000):
            ...
            for cell in self.cellListByType(self.CORE2,self.CORE2_PRIME): 
                if ((1*self.tot)+self.mid-9<= cell.id <=(1*self.tot)+self.mid+9):
                    ...
                    cell.targetVolume = cell.targetVolume + delta/4.0
                    cell.lambdaVolume=self.LamV_core*5 
        
        #set the strength of volume constraint for the cells at the end and for vitelline membrane
        for cell in self.cellListByType(self.APICAL):
            if (cell.id == (3*self.tot)+1) or (cell.id == 4*self.tot):
                cell.targetVolume=cell.volume
                cell.lambdaVolume=self.LamV*1000
        
        for cell in self.cellListByType(self.CORE,self.CORE_PRIME):
            if (cell.id == (2*self.tot)+1) or (cell.id == 3*self.tot):
                cell.targetVolume=cell.volume
                cell.lambdaVolume=self.LamV*1000
                    
        for cell in self.cellListByType(self.VM): 
                cell.targetVolume=cell.volume
                cell.lambdaVolume = 1000        
...
class cell_internal_links(SteppableBasePy):        
    ...
    def start(self):        
        ...
        CELL0 =  self.fetch_cell_by_id(1)
        CELL1 =  self.fetch_cell_by_id(48)
        CELL2 =  self.fetch_cell_by_id(95)
    ...
    def delete_internal_links(self, core_type):                 #function to delete existing internal links 
        for cell in self.cell_list_by_type(core_type):
            ...
            for cell_cmpt in cluster_cell_list:
                ...
                elif (cell_cmpt.type == self.BASAL):
                    self.focalPointPlasticityPlugin.deleteInternalFocalPointPlasticityLink(cell, cell_cmpt)
                elif (cell_cmpt.type == self.BASAL_PRIME):
                    self.focalPointPlasticityPlugin.deleteInternalFocalPointPlasticityLink(cell, cell_cmpt)
    ...
    def create_internal_links(self, core_type, basal_type):     #function to create new internal links between respective cell compartments
        for cell in self.cell_list_by_type(core_type):        
            ...
            if ((2*self.tot)+self.mid-self.n_cells <= cell.id <= (2*self.tot)+self.mid+self.n_cells):
                cluster_cell_list = self.get_cluster_cells(cell.clusterId)
                for cell_cmpt in cluster_cell_list:
                    if (cell_cmpt.type == basal_type):
                        target_distance = (cell.targetVolume + delta)/17
                       # print("45 ", target_distance, cell.targetVolume/8.5)
                        link = self.focalPointPlasticityPlugin.createInternalFocalPointPlasticityLink(cell, cell_cmpt, self.lambda_links_basal_central[int(self.profile_counter)], target_distance, 200)
                        cell_cmpt.connectivityOn = True

                    if (cell_cmpt.type == self.APICAL):
                        target_distance = self.ap_core_distance
                        link = self.focalPointPlasticityPlugin.createInternalFocalPointPlasticityLink(cell, cell_cmpt, self.lambda_links_apical_central[int(self.profile_counter)], target_distance, 200)
        
            #create links for peripheral cells
            elif ((2*self.tot)+self.mid-12 <= cell.id < (2*self.tot)+self.mid-self.n_cells):
                cluster_cell_list = self.get_cluster_cells(cell.clusterId)
                for cell_cmpt in cluster_cell_list:
                    if (cell_cmpt.type == basal_type):
                        target_distance = (cell.targetVolume + delta)/17.0
                        link = self.focalPointPlasticityPlugin.createInternalFocalPointPlasticityLink(cell, cell_cmpt, self.lambda_links_basal_peripheral[int(self.profile_counter)], target_distance, 200)
                        cell_cmpt.connectivityOn = True

                    if (cell_cmpt.type == self.APICAL):
                        target_distance = self.ap_core_distance
                        link = self.focalPointPlasticityPlugin.createInternalFocalPointPlasticityLink(cell, cell_cmpt, self.lambda_links_apical_peripheral[int(self.profile_counter)], target_distance, 200)

            #create links for peripehral cells
            elif ((2*self.tot)+self.mid+self.n_cells < cell.id <= (2*self.tot)+self.mid+12):
                cluster_cell_list = self.get_cluster_cells(cell.clusterId)
                for cell_cmpt in cluster_cell_list:
                    if (cell_cmpt.type == basal_type):
                        target_distance = (cell.targetVolume + delta)/17.0
...
class apical_constriction(SteppableBasePy):  
    ...
    def start(self):
        ...
        for cell in self.cell_list_by_type(self.APICAL):        
            if ((3*self.tot)+self.mid-10 <= cell.id <= (3*self.tot)+self.mid+9):
    ...
    def step(self,mcs):
        if (mcs < 13*self.t_relax):
            ...
            for cell in self.cell_list_by_type(self.APICAL):
                if ((3*self.tot)+self.mid-10 <= cell.id < (3*self.tot)+self.mid):
                ...
                elif ((3*self.tot)+self.mid <= cell.id <= (3*self.tot)+self.mid+9):
                    for link in self.get_focal_point_plasticity_data_list(cell): 
                        initiated_cell = link.neighborAddress
                        initiator_cell = cell  
                        if initiated_cell.id > cell.id:   
...
class metric(SteppableBasePy):
    ...
    def start(self):
        self.mid = 165                                  #cell-id of the apical compartment at the ventral midline
